...
<body class="sb-nav-fixed">
	...
	<div id="layoutSidenav_content">
		<main>
			<div class="container-fluid px-4">
				<h1 class="mt-4">Products Sizes</h1>
				<ol class="breadcrumb mb-4">
					<li class="breadcrumb-item active">Products Size</li>
				</ol>
				...
				<a href="${base }/admin/add-product-sizes"
					class="btn btn-secondary active" role="button" aria-pressed="true">Add
					New Product Size</a> <br> <br>
				<sf:form action="${base }/admin/list-product-sizes" method="GET"
					...
					<c:choose>
						<c:when test="${product_id!=0 }">
							<div style="margin-left: 95px">
								<input type="checkbox" id="haspid" class="form-check-input"
									...
									if you want to filter by product?</label>
							</div>

							<!-- <br> -->
							<sf:select disabled="false" path="product_id" name="product_id"
								id="product_id" style="outline: none;margin-left:75px;">
						...
						<c:otherwise>
							...
							<!-- <br> -->
							<sf:select disabled="true" path="product_id" name="product_id"
								id="product_id" style="outline: none;margin-left:75px;">
								<sf:options items="${products }" itemValue="id"
									itemLabel="title" />
							</sf:select>
				...
				<c:if test="${totalSizes>0 }">
					<div style="background-color: black">
						<p id="show" style="color: white; font-size: 20px; padding: 10px">Showing:
							${curPage*sizeOfPage+1}-${curPage*sizeOfPage+totalDisplay} of Total: ${totalSizes }
							kết quả</p>
					</div>
				</c:if>
			...
			<div class="table-responsive container-fluid px-4"
				...
				<input type="hidden" value="${totalPage }" id="totalPage"> <input
					...
					type="hidden" value="${product_id }" id="product_id1">
				...
				<table class="table table-hover table-bordered table-condensed">
					...
					<c:forEach var="productSize" items="${sizes }" varStatus="loop">

						<tr>
							<td id="ID">${sizeOfPage * curPage + loop.count}</td>
							<td>${productSize.size}</td>
							<td>${productSize.remaining}</td>
							<%-- <c:forEach var="product" items="${products }">
								<c:if test="${product.id==productSize.products2.id }"> --%>
							<td>${productSize.products2.title }</td>
							<%-- 	</c:if>

							</c:forEach> --%>
							<%-- <td>${productSize.created_date }</td>
							<td>${productSize.updated_date }</td> --%>
							<%-- <c:choose>
	                    			<c:when test="${ productSize.created_by!=null}">
		                    			<c:forEach var="user" items="${users }">
		                    				<c:if test="${user.id==productSize.created_by }">
                    			...
                    			<c:choose>
	                    			<c:when test="${ productSize.updated_by!=null}">
		                    			<c:forEach var="user" items="${users }">
		                    				<c:if test="${user.id==productSize.updated_by }">
		                    					<td>${user.full_name }</td>
		                    				</c:if>
		                    			</c:forEach>
	                    			</c:when>
							...
							<c:choose>
								<c:when test="${productSize.status==true}">
									<td>Active</td>
								</c:when>
								<c:otherwise>
									<td>InActive</td>
							...
							<td><a
								href="${base}/admin/edit-product-sizes/${productSize.id}"
	...
	<script type="text/javascript">
		...
		function function2() {
			return confirm('Bạn có chắc chắn muốn ẩn size này?');
		}
		$(document).on(
				"click",
				".confirmation",
				function() {
					if (function2()) {
						...
						jQuery.ajax({
							url : "/admin/product-sizes/delete",
							type : "post",
							contentType : "application/json",
							data : JSON.stringify(data),
							dataType : "json", // kieu du lieu tra ve tu controller la json
							success : function(jsonResult) {
								alert("Đã ẩn size " + jsonResult.SizeName
										+ " của product " + jsonResult.proSeo);
		...
		function function3() {
			return confirm('Bạn có chắc chắn muốn xóa size này khỏi Database?');
		}
		$(document).on(
				"click",
				".confirmation1",
				function() {
					if (function3()) {
						...
						jQuery.ajax({
							url : "/admin/product-sizes/removeFromDB",
							type : "post",
							contentType : "application/json",
							data : JSON.stringify(data),
							dataType : "json", // kieu du lieu tra ve tu controller la json
							...
							jQuery
									.ajax({
										...
										success : function(jsonResult) {
											...
											if (jsonResult.totalSizes >= 1) {
												$("#show")
														.text(
																"Showing: "
																		+ (jsonResult.curPage * jsonResult.sizeOfPage + 1)
																		+ "-"
																		+ (jsonResult.curPage * jsonResult.sizeOfPage + jsonResult.totalDisplay)
																		+ " of Total: "
																		+ jsonResult.totalSizes
																		+ " kết quả");
											} else {
												$("#show")
														.text(
