...
<body class="sb-nav-fixed">
				...
				<sf:form action="${base }/admin/list-categories" method="GET"
					...
					<sf:select path="filter" name="filter" id="filter"
						...
						<option value="all" ${filter == 'all' ? 'selected' : ''}>Tất
							cả</option>
						<option value="parent" ${filter == 'parent' ? 'selected' : ''}>Danh
							mục cha</option>
						<option value="children" ${filter == 'children' ? 'selected' : ''}>Danh
							mục con</option>
				...
				<c:if test="${totalCategories>0 }">
					<div style="background-color: black">
						<p id="show" style="color: white; font-size: 20px; padding: 10px">Showing:
							...
							Total: ${totalCategories } kết quả</p>
					</div>
				</c:if>
			</div>
					...
					<c:forEach var="category" items="${categories}" varStatus="loop">
						...
						<tr>
							...
							<td><fmt:formatDate pattern="dd-MM-yyyy HH:mm:ss"
									value="${category.created_date}" /></td>
							<td><fmt:formatDate pattern="dd-MM-yyyy HH:mm:ss"
									value="${category.updated_date}" /></td>
							...
							<td><a href="${base}/admin/edit-category/${category.seo}"
	...
	<script type="text/javascript">
		...
		function function2() {
			return confirm('Bạn có chắc chắn muốn ẩn category này?');
		}
			...
			if (function2()) {
				...
				jQuery.ajax({
					...
					success : function(jsonResult) {
						alert("Đã ẩn category có tên " + jsonResult.cateName);
		...
		function function3() {
			return confirm('Bạn có chắc chắn muốn xóa danh mục này khỏi Database?');
		}
		$(document).on(
				"click",
				".confirmation1",
				function() {
					if (function3()) {
						...
						jQuery.ajax({
							url : "/admin/category/removeFromDB",
							type : "post",
							contentType : "application/json",
							data : JSON.stringify(data),
							dataType : "json", // kieu du lieu tra ve tu controller la json
		...
		$(document)
				.on(
						...
						function() {
							...
							window.history.pushState(null, null,
									"/admin/list-categories?page=" + (page + 1)
											+ "&keyword=" + $("#keyWord").val()
											+ "&sort=" + $("#sort").val()
											+ "&filter=" + $("#filter").val());
							var data = {
							...
							jQuery
									.ajax({
										...
										success : function(jsonResult) {
											...
											if (jsonResult.totalCategories >= 1) {
												$("#show")
														.text(
																"Showing: "
																		+ (jsonResult.curPage
