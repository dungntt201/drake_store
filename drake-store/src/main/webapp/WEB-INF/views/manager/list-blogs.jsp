...
<body class="sb-nav-fixed">
	...
	<div id="layoutSidenav_content">
		<main>
			<div class="container-fluid px-4">
				...
				<a href="${base }/admin/add-blogs" class="btn btn-secondary active"
					role="button" aria-pressed="true">Add New Blog</a> <br> <br>
				<sf:form action="${base }/admin/list-blogs" method="GET"
				...
				<table class="table table-hover table-bordered table-condensed">
					...
					<c:forEach var="blog" items="${blogs }" varStatus="loop">
						...
						<tr>
							...
							<td><img alt="" src="${base }/upload/${blog.avatar}">
							</td>

							<%-- <c:choose>
								<c:when test="${blog.author!=null }"> --%>
							<td>${blog.author}</td>
							<%-- </c:when>
								<c:otherwise>
									<td></td>
							...
							<%-- <c:choose>
								<c:when test="${blog.updated_by!=null }">
									<c:forEach var="user" items="${users }">
										<c:if test="${user.id==blog.updated_by}">
							...
							<td><fmt:formatDate pattern="dd-MM-yyyy HH:mm:ss"
									value="${blog.created_date}" /></td>
							<%-- <td><fmt:formatDate pattern="dd-MM-yyyy HH:mm:ss"
									value="${blog.updated_date}" /></td> --%>
							...
							<td><a href="${base}/admin/edit-blog/${blog.seo}"
	...
	<script type="text/javascript">
		...
		function function2() {
			return confirm('Bạn có chắc chắn muốn ẩn blog này?');
		}
			...
			if (function2()) {
				...
				jQuery.ajax({
					...
					success : function(jsonResult) {
						alert("Đã ẩn blog có tên " + jsonResult.blogName);
